...
image_name="${MUXCHAT_SSH_FIXTURE_IMAGE:-muxchat-ssh-fixture:latest}"
container_name="${MUXCHAT_SSH_FIXTURE_CONTAINER:-muxchat-ssh-fixture}"
fixture_user="muxchat"
fixture_secret="${MUXCHAT_SSH_FIXTURE_SECRET:-muxchat}"
...
docker build --build-arg "MUXCHAT_FIXTURE_SECRET=${fixture_secret}" -t "${image_name}" "${fixture_dir}"
cleanup
trap cleanup EXIT
...
(
  cd "${gateway_dir}"
  env \
    MUXCHAT_TEST_SSH_HOST=127.0.0.1 \
    MUXCHAT_TEST_SSH_PORT="${fixture_port}" \
    MUXCHAT_TEST_SSH_USER="${fixture_user}" \
    MUXCHAT_TEST_SSH_PASSWORD="${fixture_secret}" \
